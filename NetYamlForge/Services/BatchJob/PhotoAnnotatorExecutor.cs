...
/// <summary>
/// processing_queue からフォトを取り出し、Anthropic/Gemini/Ollama で画像標注を実行する。
...
public class PhotoAnnotatorExecutor : IBatchStepHandler
{
        ...
        string absolutePath, string provider, string? projectName, CancellationToken ct)
    {
        return provider.ToLowerInvariant() switch
        {
            ...
            "antigravity"  => await AnnotateWithGeminiCliAsync(absolutePath, ct),  // fallback to gemini_cli
            _              => await AnnotateWithAnthropicAsync(absolutePath, projectName, ct),
        };
    }
        ...
        string absolutePath, CancellationToken ct)
    {
        ...
        {
            FileName = "gemini",
            Arguments = $"-p \"{escapedPrompt}\" --yolo",
        ...
        catch (OperationCanceledException)
        {
            ...
            _logger.LogError("Gemini CLI timed out during photo annotation");
            return null;
        }

        if (process.ExitCode != 0)
        {
            _logger.LogError("Gemini CLI exited with code {Code}", process.ExitCode);
            return null;
        }
