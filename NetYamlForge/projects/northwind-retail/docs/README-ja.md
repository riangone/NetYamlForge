...
dotnet run -- --scaffold-entities --project=northwind-retail

# 2. ブラウザで確認
# → http://localhost:5000/northwind-retail
# → http://localhost:5000/northwind-retail/Dashboard
```
...
navigation:
  items:
    - label: My Page
      url: /northwind-retail/Page/MyPage
      icon: 📋
```
...
| フックを追加したい | `dotnet run -- --scaffold-hook --name=MyHook --project=northwind-retail` |
