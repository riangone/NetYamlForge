...
dotnet run -- --scaffold-entities --project=task-management

# 2. ブラウザで確認
# → http://localhost:5000/task-management
# → http://localhost:5000/task-management/Dashboard
```
...
navigation:
  items:
    - label: My Page
      url: /task-management/Page/MyPage
      icon: 📋
```
...
| フックを追加したい | `dotnet run -- --scaffold-hook --name=MyHook --project=task-management` |
