...
dotnet run -- --scaffold-entities --project=golden-template

# 2. ブラウザで確認
# → http://localhost:5000/golden-template
# → http://localhost:5000/golden-template/Dashboard
```
...
navigation:
  items:
    - label: My Page
      url: /golden-template/Page/MyPage
      icon: 📋
```
...
| フックを追加したい | `dotnet run -- --scaffold-hook --name=MyHook --project=golden-template` |
