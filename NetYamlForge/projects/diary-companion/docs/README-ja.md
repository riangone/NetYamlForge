...
dotnet run -- --scaffold-entities --project=diary-companion

# 2. ブラウザで確認
# → http://localhost:5000/diary-companion
# → http://localhost:5000/diary-companion/Dashboard
```
...
navigation:
  items:
    - label: My Page
      url: /diary-companion/Page/MyPage
      icon: 📋
```
...
| フックを追加したい | `dotnet run -- --scaffold-hook --name=MyHook --project=diary-companion` |
