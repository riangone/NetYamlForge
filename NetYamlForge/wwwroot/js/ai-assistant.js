...
(function() {
    ...
    // PathBase 获取（从 _Layout.cshtml 注入的 window.NetYamlForgeConfig）
    function getPath(path) {
        const base = (window.NetYamlForgeConfig && window.NetYamlForgeConfig.pathBase) || '';
        if (path.startsWith(base)) return path; // 防止重复添加
        return base + path;
    }

    // 配置
    const CONFIG = {
        apiBaseUrl: getPath('/api/ai'),
        signalRUrl: getPath('/aiProgressHub'),
    ...
    function initSignalR() {
        ...
        const signalRSources = [
            getPath('/lib/microsoft/signalr/dist/browser/signalr.min.js'),
            getPath('/lib/signalr/signalr.min.js'),
