...
namespace NetYamlForge.AI.Services.ToolValidation;
...
public class ToolCallValidator
{
        ...
        string projectId)
    {
        ...
        // 允许的实体列表(默认)
        var allowedEntities = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
        {
            "vehicles",
            "sales_leads",
            "customers",
    ...
    private ToolValidationResult ValidateSqlSafety(JsonNode toolCall)
    {
        ...
        if (!string.IsNullOrEmpty(entityName) && !_safetyGuard.IsValidIdentifier(entityName))
        {
            _logger.LogWarning("[SqlSafetyGuard] 实体名校验失败:{Entity}", entityName);
            return ToolValidationResult.Fail($"实体名包含不安全字符:{entityName}");
        }

        // 3.2 验证 Action(防止 SQL 注入操作)
        if (!string.IsNullOrEmpty(action) && !_safetyGuard.IsValidIdentifier(action))
        {
            _logger.LogWarning("[SqlSafetyGuard] Action 校验失败:{Action}", action);
        ...
        if (toolCall["filters"] is JsonArray filtersArray)
        {
            foreach (var filter in filtersArray)
            {
                if (filter is JsonObject filterObj && filterObj.TryGetPropertyValue("field", out var fieldNode))
                {
                    ...
                    if (!string.IsNullOrEmpty(fieldName) && !_safetyGuard.IsValidIdentifier(fieldName))
                    {
                        _logger.LogWarning("[SqlSafetyGuard] 过滤器字段校验失败:{Field}", fieldName);
                ...
                if (filter is JsonObject filterObj2 && filterObj2.TryGetPropertyValue("orderBy", out var orderByNode))
                {
                    if (orderByNode is JsonObject orderByObj && orderByObj.TryGetPropertyValue("field", out var orderFieldNode))
                    {
                        ...
                        if (!string.IsNullOrEmpty(orderField) && !_safetyGuard.IsValidIdentifier(orderField))
                        {
                            _logger.LogWarning("[SqlSafetyGuard] 排序字段校验失败:{Field}", orderField);
        ...
        if (_safetyGuard.IsUnsafeToken(fullJson))
        {
