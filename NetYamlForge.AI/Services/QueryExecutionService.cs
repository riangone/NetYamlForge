...
public class QueryExecutionService
{
    ...
    {
        // 1. 检查 SQL 注入标记
        if (_safetyGuard.IsUnsafeToken(sql))
        {
        ...
        CancellationToken ct = default)
    {
        ...
        var tableName = meta?.TableName ?? query.Entity;
        ...
        {
            Filters = filters.ToDictionary(kv => kv.Key, kv => (object?)kv.Value),
            Limit = query.Top ?? 100,
            Columns = query.Select.Count > 0 ? query.Select : new List<string>()
        };

        if (query.OrderBy != null)
        {
            queryParams.OrderBy.Add($"{query.OrderBy.Field} {query.OrderBy.Dir}");
        }

        // 执行查询
        var items = (await _queryExecutor.ExecuteQueryAsync(query.Entity, queryParams, ct)).ToList();

        // 获取总数
        var countParams = new QueryParameters { Filters = queryParams.Filters };
        var total = (int)await _queryExecutor.CountAsync(query.Entity, countParams, ct);
    ...
    {
        var filters = BuildFilters(query.Filters);
        var queryParams = new QueryParameters { Filters = filters.ToDictionary(kv => kv.Key, kv => (object?)kv.Value) };

        var count = (int)await _queryExecutor.CountAsync(query.Entity, queryParams, ct);
    ...
    {
        return !string.IsNullOrEmpty(column) && _safetyGuard.IsValidIdentifier(column);
    }
