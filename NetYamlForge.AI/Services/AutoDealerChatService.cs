        ...
        string? context)
    {
        ...
        switch (intent)
        {
            ...
            case "vehicle_search" or "vehicles" when dataRows?.Count > 0:
            {
                components.Add(new CardCarousel(
                    Title: $"検索結果（{dataRows.Count}件）",
                    Items: dataRows.Select(r => {
                        var id = r.GetValueOrDefault("vehicle_id", r.GetValueOrDefault("id", ""));
                        var maker = r.GetValueOrDefault("maker", "");
                        var brand = r.GetValueOrDefault("brand", "");
                        var model = r.GetValueOrDefault("model", "");
                        var grade = r.GetValueOrDefault("grade", "");
                        var year = r.GetValueOrDefault("year", "");
                        var price = r.GetValueOrDefault("price", "");
                        var mileage = r.GetValueOrDefault("mileage", "");
                        var fuelType = r.GetValueOrDefault("fuel_type", "");
                        var transmission = r.GetValueOrDefault("transmission", "");
                        var color = r.GetValueOrDefault("color", "");
                        var status = r.GetValueOrDefault("status", r.GetValueOrDefault("stock_status", ""));
                        var imageUrl = r.GetValueOrDefault("image_url", "");

                        // タイトル：メーカー + 車種名
                        var title = !string.IsNullOrEmpty(maker) ? $"{maker} {model}" : 
                                    !string.IsNullOrEmpty(brand) ? $"{brand} {model}" : model;
                        
                        // サブタイトル：グレード + 年式 + 価格
                        var subtitleParts = new List<string>();
                        if (!string.IsNullOrEmpty(grade)) subtitleParts.Add(grade);
                        if (!string.IsNullOrEmpty(year)) subtitleParts.Add($"{year}年式");
                        if (!string.IsNullOrEmpty(price))
                        {
                            var priceVal = decimal.TryParse(price, out var p) ? p : 0;
                            subtitleParts.Add(priceVal >= 10000 
                                ? $"¥{(priceVal / 10000):F0}万円" 
                                : $"¥{priceVal:N0}");
                        }
                        ...
                        if (!string.IsNullOrEmpty(mileage)) metadata["走行距離"] = $"{mileage}km";
                        if (!string.IsNullOrEmpty(fuelType))
                        {
                            var fuelLabel = fuelType.ToLower() switch
                            {
                                "gasoline" => "ガソリン",
                                "diesel" => "ディーゼル",
                                "hybrid" => "ハイブリッド",
                            ...
                            metadata["燃料"] = fuelLabel;
                        }
                        if (!string.IsNullOrEmpty(transmission)) metadata["トランスミッション"] = transmission;
                        if (!string.IsNullOrEmpty(color)) metadata["カラー"] = color;

                        // タグ（検索条件表示用）
                        var tags = new List<string>();
                        if (!string.IsNullOrEmpty(status))
                        {
                            var statusLabel = status.ToLower() switch
                            {
                                "available" => "在庫あり",
                                "reserved" => "予約済み",
                                "sold" => "売約済",
                        ...
                        return new CardItem(
                            ...
                            Subtitle: subtitle,
                            ImageUrl: imageUrl,
                            BadgeLabel: tags.Count > 0 ? tags[0] : null,
                            BadgeStyle: GetBadgeStyle(status),
                            Metadata: metadata,
                            Tags: tags,
                            Actions: new List<CardAction>
                            {
                                new("詳細", "postback", $"車両ID {id} の詳細を教えて"),
                                new("試乗予約", "postback", $"車両ID {id} を試乗予約したい"),
                                new("見積もり", "postback", $"車両ID {id} の見積もりを依頼したい"),
                            }
                        );
                    }).ToList()
                ));
