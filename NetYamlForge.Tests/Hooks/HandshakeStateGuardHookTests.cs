// 責務: HandshakeStateGuardHook のユニットテスト。
...
public class HandshakeStateGuardHookTests
{
    // ─── ヘルパー ───────────────────────────────────────────
    ...
    {
        var sut = new HandshakeStateGuardHook(
            Microsoft.Extensions.Logging.Abstractions.NullLogger<HandshakeStateGuardHook>.Instance);
    ...
    public void Name_IsCorrectYamlKey()
    {
        var sut = new HandshakeStateGuardHook(
            Microsoft.Extensions.Logging.Abstractions.NullLogger<HandshakeStateGuardHook>.Instance);
        Assert.Equal("handshake_state_guard", sut.Name);
    }
}