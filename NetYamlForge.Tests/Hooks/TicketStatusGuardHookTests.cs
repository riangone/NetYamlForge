// 責務: TicketStatusGuardHook のユニットテスト。
// 正常系・異常系・フィールド欠損の3パターンを最低限テストする。

using NetYamlForge.Services.Hooks;
using NetYamlForge.Projects.GoldenTemplate.Hooks;
using Xunit;

namespace NetYamlForge.Tests.Hooks;

public class TicketStatusGuardHookTests
{
    // ─── ヘルパー ───────────────────────────────────────────
    ...
    {
        var sut = new TicketStatusGuardHook(
            Microsoft.Extensions.Logging.Abstractions.NullLogger<TicketStatusGuardHook>.Instance);
    ...
    public void Name_IsCorrectYamlKey()
    {
        var sut = new TicketStatusGuardHook(
            Microsoft.Extensions.Logging.Abstractions.NullLogger<TicketStatusGuardHook>.Instance);
        Assert.Equal("ticket_status_guard", sut.Name);
    }
}