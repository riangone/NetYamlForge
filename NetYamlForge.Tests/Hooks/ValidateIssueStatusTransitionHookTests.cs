// 責務: ValidateIssueStatusTransitionHook のユニットテスト。
// 正常系・異常系・フィールド欠損の3パターンを最低限テストする。

using NetYamlForge.Services.Hooks;
using NetYamlForge.Projects.Redmine.Hooks;
using Xunit;

namespace NetYamlForge.Tests.Hooks;

public class ValidateIssueStatusTransitionHookTests
{
    // ─── ヘルパー ───────────────────────────────────────────
    ...
    {
        var sut = new ValidateIssueStatusTransitionHook(
            Microsoft.Extensions.Logging.Abstractions.NullLogger<ValidateIssueStatusTransitionHook>.Instance);
    ...
    public void Name_IsCorrectYamlKey()
    {
        var sut = new ValidateIssueStatusTransitionHook(
            Microsoft.Extensions.Logging.Abstractions.NullLogger<ValidateIssueStatusTransitionHook>.Instance);
        Assert.Equal("validate_issue_status_transition", sut.Name);
    }
}