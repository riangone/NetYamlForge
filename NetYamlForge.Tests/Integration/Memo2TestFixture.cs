// ファイル概要: memo2 プロジェクトの E2E テスト用ファクトリです。
...
public sealed class Memo2TestFixture : WebApplicationFactory<Program>
{
    public const string ProjectName = "memo2";
    ...
    public Memo2TestFixture()
    {
        var appProjectDir = FindAppProjectDirectory();
        TempContentRoot = Path.Combine(Path.GetTempPath(), "nyf-memo2-e2e", Guid.NewGuid().ToString("N"));
